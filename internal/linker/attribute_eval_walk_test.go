...
// Test_EvalWalk_EmitsLetBoundMaxViolation proves the walker validates
...
func Test_EvalWalk_EmitsLetBoundMaxViolation(t *testing.T) {
	src := `module main
import "std"
import "std/posix"
import "std/local"
...
// Test_EvalWalk_AcceptsValidLetBound proves the walker emits nothing
// when the resolved value satisfies all attributes - exactly the
// regression that issue-184 is fixing for real configs.
func Test_EvalWalk_AcceptsValidLetBound(t *testing.T) {
	src := `module main
import "std"
import "std/posix"
import "std/local"
...
// Test_EvalWalk_EmitsLiteralMaxViolation proves the walker behaves the same
// as the AST-walker for literal arguments: it validates and emits the
// same violation.
func Test_EvalWalk_EmitsLiteralMaxViolation(t *testing.T) {
	src := `module main
import "std"
import "std/posix"
import "std/local"
