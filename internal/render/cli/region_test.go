...
func Test_RegionLines_CapsRunningWithOverflow(t *testing.T) {
...
// empty.
func Test_RegionLines_ReturnsEmptyWhenIdle(t *testing.T) {
	c := New(Options{Stdout: &bytes.Buffer{}, ForceASCII: true}, nil)
	c.isTTY = true
	c.width = 200
...
func Test_RegionLines_KeepsFooterUnderHeightCap(t *testing.T) {
	c := New(Options{Stdout: &bytes.Buffer{}, ForceASCII: true}, nil)
	c.isTTY = true
	c.width = 200
	c.height = 5
...
// Test_LiveRegion_MatchesGolden pins one rendered live-region frame: spinner, padded
...
func Test_LiveRegion_MatchesGolden(t *testing.T) {
