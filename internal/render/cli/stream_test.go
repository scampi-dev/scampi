...
// Test_Stream_MatchesGolden locks the check/apply block format (glyph-led header, railed
...
func Test_Stream_MatchesGolden(t *testing.T) {
	combos := []struct {
		name string
		v    signal.Verbosity
	}{
...
func Test_Stream_DrawsLiveRegionAroundBlocks(t *testing.T) {
	at := time.Unix(1000, 0)
	c, buf := newTTYCLI(at)
	s := newStreamSink(c)
