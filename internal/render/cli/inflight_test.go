...
func Test_Inflight_TracksRunningPerLane(t *testing.T) {
	f := newInflight()
	base := time.Unix(0, 0)
...
func Test_Inflight_ReportsIdleWhenAllFinished(t *testing.T) {
	f := newInflight()
	base := time.Unix(0, 0)
	f.begin(sref(0, "web", 0), base)
	f.finish(sref(0, "web", 0), false)
...
func Test_Inflight_SumsProgressAcrossLanes(t *testing.T) {
	f := newInflight()
	base := time.Unix(0, 0)
...
func Test_Inflight_CountsHooksSeparately(t *testing.T) {
	f := newInflight()
	base := time.Unix(0, 0)
